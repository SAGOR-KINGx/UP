    ...
    return async function (event, message) {
        ...
        */
        ...
        async function onStart() {
            ...
            // —————————————— CHECK USE BOT —————————————— //
            const config = global.GoatBot.config;
            const isNoPrefix = config.noPrefix || false;
            let usedPrefix = body && body.startsWith(prefix);
            ...
            const bodyWithoutPrefix = usedPrefix ? body.slice(prefix.length).trim() : body.trim();
            const args = bodyWithoutPrefix.split(/ +/);
            // ————————————  CHECK HAS COMMAND ——————————— //
            let commandName = args.shift().toLowerCase();
            let command = GoatBot.commands.get(commandName) || GoatBot.commands.get(GoatBot.aliases.get(commandName));
            
            if (!command && !usedPrefix) return; // If no prefix and not a command, ignore "OK bye" etc.
            ...
            if (!command) {
                if (!hideNotiMessage.commandNotFound) {
                    ...
                    const commandNames = Array.from(GoatBot.commands.keys());
                    const aliases = Array.from(GoatBot.aliases.keys());
                    const allCommands = [...new Set([...commandNames, ...aliases])];
                    const matches = stringSimilarity.findBestMatch(commandName, allCommands);
                    const bestMatch = matches.bestMatch;

                    if (bestMatch.rating >= 0.3) {
                        return await message.reply(
                            utils.getText({ lang: langCode, head: "handlerEvents" }, "commandNotFoundWithSuggestion", commandName, prefix, bestMatch.target)
                        );
                    }
